...
export async function render(container) {
    try {
        const response = await fetch(`${apiUrl}/admin/userreports/reports?status=Resolved`, {
        //const response = await fetch(`https://localhost:7128/admin/userreports/reports?status=Resolved`, {
            method: 'GET',
            headers: {
        ...
        container.innerHTML = reports.map(r => `
            <div class="report" data-id="${r.id}" style="width: 60%; margin: 2em auto; padding: 1em; border: 1px solid #ccc; border-radius: 8px;">
                ...
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.5em;">
                    ...
                    <div style="color: #555;"><strong>Gjelder:</strong> ${r.tabell || 'Ukjent'}</div>
                </div>            
                <textarea placeholder="Feedback..." rows="4" style="width: 100%; resize: vertical; margin-bottom: 0.5em;">${r.feedback || ''}</textarea>

                <button class="button-brown save-btn" data-status="Resolved">Lagre</button>
                <button class="button-brown save-btn" data-status="inProgress" style="margin-left: 0.5em;">Lagre og flytt til Under Behandling</button>
            </div>
        `).join('');
        ...
        container.querySelectorAll('.save-btn').forEach(button => {
            button.addEventListener('click', async () => {
                ...
                try {
                    ...
                    render(container); // reload lokalt
                } catch (err) {
