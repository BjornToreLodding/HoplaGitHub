...
export async function render(container) {
    try {
        ...
        //const response = await fetch(`https://localhost:7128/admin/userreports/reports?status=new`, {
                method: 'GET',
                headers: {
            ...
            container.innerHTML = reports.map(r => `
                <div class="report" data-id="${r.id}" style="width: 60%; margin: 2em auto; padding: 1em; border: 1px solid #ccc; border-radius: 8px;">
                    ...
                    <textarea placeholder="Feedback..." rows="4" style="width: 100%; resize: vertical; margin-bottom: 0.5em;"></textarea>
                    
                    <button class="button-brown save-btn" data-status="inProgress">Lagre og marker Under Behandling</button>
            ...
            container.querySelectorAll('.save-btn').forEach(button => {
                button.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        render(container); // ← Refresher siden når alert-boksen lukkes
                    } catch (err) {
