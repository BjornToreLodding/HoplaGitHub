...
public class UserRelationsController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateRelation([FromBody] UserRelationRequest request)
    {
        ...
        if (status is not ("FOLLOWING" or "PENDING" or "BLOCK"))
            return BadRequest(new { message = "Ugyldig status. Må være FOLLOWING, PENDING eller BLOCK." });
        ...
        switch (status)
        {
            ...
            case "FOLLOWING":
                ...
                if (relations.Any(r =>
                    r.Status == "FRIENDS" ||
                    (r.Status == "BLOCK" && r.FromUserId == request.TargetUserId && r.ToUserId == userId)))
                    return BadRequest(new { message = "Kan ikke følge bruker grunnet blokkering eller vennskap" });

                // Hvis bruker har blokkert andre, fjern relasjonen før FOLLOWING
                if (relations.Any(r =>
                    r.Status == "BLOCK" && r.FromUserId == userId && r.ToUserId == request.TargetUserId))
                    _context.UserRelations.RemoveRange(relations);
                break;
        }
        ...
        var readable = status switch
        {
            "PENDING" => "Venneforespørsel sendt",
            "BLOCK" => "Bruker blokkert",
            "FOLLOWING" => "Du følger nå brukeren",
    ...
    */
    ...
    public async Task<IActionResult> UpdateRelationStatus([FromBody] UserRelationRequest request)
    {
        ...
        var allowedStatuses = new[] { "PENDING", "FOLLOWING", "BLOCK", "FRIENDS" };

        if (!allowedStatuses.Contains(status))
            return BadRequest(new { message = "Ugyldig statusverdi." });
        ...
        var currentStatus = relation.Status;

        // ✅ Godta venneforespørsel
        if (status == "FRIENDS")
        {
            if (currentStatus != "PENDING" || relation.ToUserId != userId)
        ...
        // ✅ Hvis FRIENDS → BLOCK, bytt retning og status
        else if (status == "BLOCK" && currentStatus == "FRIENDS")
        {
        ...
        {
            return BadRequest(new { message = $"Kan ikke endre en FRIENDS-relasjon til {status}. Fjern først vennskapet." });
        }

        await _context.SaveChangesAsync();
        return Ok(new { message = $"Status oppdatert til {status}" });
    }


    //Denne mangler mulighet til å fjerne following
    [Authorize]
    [HttpDelete]
    public async Task<IActionResult> DeleteRelation([FromBody] UserRelationRequest request)
    {
        ...
        var relation = await _context.UserRelations.FirstOrDefaultAsync(r =>
            // FOLLOWING og BLOCK: kun én vei (fra meg til deg)
            (r.Status == "FOLLOWING" || r.Status == "BLOCK") &&
            r.FromUserId == userId && r.ToUserId == request.TargetUserId

            // PENDING og FRIENDS: kan fjernes begge veier
            || (r.Status == "PENDING" || r.Status == "FRIENDS") &&
            (
                (r.FromUserId == userId && r.ToUserId == request.TargetUserId) ||
                (r.FromUserId == request.TargetUserId && r.ToUserId == userId)
            )
