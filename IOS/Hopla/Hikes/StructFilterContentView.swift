...
struct FilterContentView: View {
    ...
    // Fetch trails made by friends and following
...
struct HandleToggleFavorite {
    /// A binding to the array of hikes, so we can update it in place
    let hikes: Binding<[Hike]>
    /// The service that actually performs the API call
    let viewModel: HikeService
    
    /// To heart or un-heart a trail
...
struct MapContainerView: UIViewRepresentable {
    ...
    func updateUIView(_ mapView: GMSMapView, context: Context) {
        ...
        for hike in hikes {
            print("Hike:", hike.name, hike.latitude ?? 0, hike.longitude ?? 0)
        }
...
// MARK: - To display trails with coordinates on map
